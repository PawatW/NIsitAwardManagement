...
import { 
  ...
  ChevronLeft, Info, GraduationCap, Phone, Calendar, Users, MapPin, ArrowRight, 
  Edit3
} from "lucide-react"; 
import api from "../../../../../lib/axios"; 
import { useAuth } from "../../../../../context/AuthContext";

// Backend RequestResponse 
...
export default function ApplicationReviewPage() {
  ...
  // States สำหรับ Modal เปลี่ยนประเภทรางวัล
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Parse additional_data & form_structure if needed
        if (typeof responseData.additional_data === 'string') {
          try { responseData.additional_data = JSON.parse(responseData.additional_data); } catch(e) {}
        }
        if (responseData.award && typeof responseData.award.form_structure === 'string') {
          try { responseData.award.form_structure = JSON.parse(responseData.award.form_structure); } catch(e) {}
        }
  ...
  const loadCategories = async () => {
    try {
      ...
      let catsData = res.data.data || res.data || [];
      ...
      console.error("❌ ไม่สามารถดึงข้อมูลประเภทรางวัลได้", error);
    }
  };
  ...
  const handleChangeCategorySubmit = async () => {
    ...
    try {
      ...
      await api.post(`/api/v1/requests/${applicationId}/change-category`, { 
      ...
      router.push('/staff-page/app/review');
  ...
  const isPending = rawStatus === "SUBMITTED" ||  rawStatus === "AWARD_CHANGED" || rawStatus === "PENDING_COMMITTEE";
  const isRejected = rawStatus.includes("REJECTED"); 
  const isApproved = rawStatus === "APPROVED_PENDING_PRESIDENT" 
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 mt-6">
      {/* Top Bar */}
          ...
          <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
            <div className="flex justify-between items-center border-b border-gray-100 pb-4">
              ...
              {!isRejected && !isApproved && (
                <button 
                  onClick={loadCategoriesForChange}
                  className="text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors border border-blue-100"
                >
                  <Edit3 size={14} /> เปลี่ยนรางวัล
                </button>
              )}
            </div>
                  ...
                  <div key={idx} className="p-6 bg-gray-50 rounded-2xl border border-gray-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(item).map(([k, v]: any) => {
                        ...
                        return (
                          <div key={k}>
                            <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1.5">{getFieldLabel(k)}</p>
                            <div className="bg-white border border-gray-200 rounded-xl p-3 text-sm font-medium text-gray-800 whitespace-pre-wrap min-h-[3rem]">
                              {v !== undefined && v !== null && v !== "" ? String(v) : <span className="text-gray-400 italic">ไม่ได้ระบุข้อมูล</span>}
                            </div>
                          </div>
                        )
                      })}
          ...
          <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-4">
            ...
            <div className="p-6 bg-green-50/30 rounded-2xl border border-green-50">
               <p className="text-gray-700 leading-relaxed whitespace-pre-line text-sm font-medium">
                 {statementContent !== "No statement provided." ? statementContent : "ไม่ได้ระบุคำชี้แจงเพิ่มเติม"}
               </p>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                          ...
                          <p className="font-bold text-slate-800 text-lg">{data?.award?.name || '-'}</p>
                      </div>
                      ...
                      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                            ...
                            <div className="space-y-4">
                              {specificAnswers.map((item: any, idx: number) => (
                                <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Entry #{idx + 1}</p>
                                  <div className="space-y-3">
                                    {Object.entries(item).map(([k, v]: any) => {
                                      if (k === 'statement_content') return null;
                                      return (
                                        <div key={k}>
                                          <p className="text-[10px] text-slate-400 font-bold mb-1">{getFieldLabel(k)}</p>
                                          <p className="text-sm font-medium text-slate-800 break-words bg-white p-2 rounded-md border border-slate-100">{String(v)}</p>
                                        </div>
                                      )
                                    })}
                                  </div>
                          ...
                          ) : Object.keys(specificAnswers || {}).filter(k => k !== 'items' && k !== 'statement_content').length > 0 ? (
                              <div className="space-y-3">
                                  {Object.entries(specificAnswers).map(([k, v]: any) => {
                      ...
                      {/* เอกสารแนบ — ใช้ data.documents ที่โหลดมาแล้วจากหน้าหลัก */}
                      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">เอกสารแนบ</p>
                          {data?.documents && data.documents.length > 0 ? (
                                  ...
                                  {data.documents.map((doc, i) => (
                                      <div key={i} className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-100">
                                          <div className="flex items-center gap-2.5 overflow-hidden">
                                              ...
                                              <div className="min-w-0">
                                                  <p className="text-xs font-bold text-slate-700 truncate">{doc.original_name || "Document"}</p>
                                                  <p className="text-[10px] text-slate-400">
                                                      {typeof doc.size === "number" ? `${(doc.size / 1024).toFixed(2)} KB` : "-"}
                                                  </p>
                                              </div>
                                          </div>
